
<h1 align="center">Hi 👋, I'm John Marlon Montoya Escobar</h1>

<h3 align="center">
Senior Full Stack Software Engineer specialized in scalable backend systems,
clean architecture and modern TypeScript ecosystems.
</h3>

<p align="center">
  <img 
    src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=2500&pause=1000&color=3B82F6&center=true&vCenter=true&width=900&lines=Senior+Full+Stack+Software+Engineer;Backend+Architecture+%26+Scalable+Systems;TypeScript+%7C+Node.js+%7C+NestJS+%7C+React;Clean+Architecture+Advocate;AI-Enhanced+Engineering+Workflows"
    alt="Typing SVG"
  />
</p>

<h3 align="center">
⚡ Building software that survives growth, complexity and time.
</h3>

<p align="center">
Building enterprise-grade applications with Node.js, NestJS, React, Vue.js and cloud-oriented solutions.
</p>

<p align="center">
  <a href="https://github.com/jhonmarlon">
    <img src="https://komarev.com/ghpvc/?username=jhonmarlon&label=Profile%20Views&color=0e75b6&style=flat" alt="profile views" />
  </a>
</p>

---

## 🚀 About Me

- 🧠 Backend-oriented Full Stack Engineer with 8+ years of experience
- ⚙️ Specialized in scalable systems, APIs and clean architecture
- 🏗️ Strong focus on maintainability, SOLID principles and modular architectures
- ☁️ Experience building cloud-oriented enterprise applications
- 🤖 Exploring AI-assisted engineering workflows and developer productivity
- 🎯 Passionate about system design, software architecture and scalability
- 🌎 Based in Medellín, Colombia

---

<h2 align="left">⚡ Tech Stack</h2>

<h3>Backend</h3>

<p>
  <img src="https://skillicons.dev/icons?i=nodejs,ts,nestjs,express" />
</p>

<h3>Frontend</h3>

<p>
  <img src="https://skillicons.dev/icons?i=react,vue,js,html,css" />
</p>

<h3>Databases</h3>

<p>
  <img src="https://skillicons.dev/icons?i=postgres,mongodb,mysql" />
</p>

<h3>DevOps & Cloud</h3>

<p>
  <img src="https://skillicons.dev/icons?i=docker,aws,nginx,git,github,linux" />
</p>

---

## 🏛️ Engineering Philosophy

I believe software should be:

- Scalable enough to evolve over time
- Maintainable for future developers
- Designed with clear architectural boundaries
- Focused on business value and developer productivity
- Built using clean engineering practices instead of temporary fixes

> Good software is not only about making things work.  
> It's about making systems sustainable.

---

## 🎯 Areas of Expertise

- Backend Architecture
- REST API Design
- Modular Monoliths
- Enterprise Software Development
- Authentication & Authorization Systems
- Scalable Web Applications
- Database Design
- Docker-Based Workflows
- Software Integrations
- Educational Platforms & LMS Solutions
- AI-Assisted Development Workflows

---

<h2 align="left">📊 GitHub Stats</h2>

<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=jhonmarlon&show_icons=true&theme=tokyonight&hide_border=true" />
  
  <img height="170" src="https://github-readme-streak-stats.herokuapp.com/?user=jhonmarlon&theme=tokyonight&hide_border=true" />
</p>

<p align="center">
  <img 
    src="https://github-readme-stats.vercel.app/api/top-langs/?username=jhonmarlon&layout=compact&theme=tokyonight&hide_border=true"
  />
</p>

---

## 🧩 Featured Projects

### 🏗️ Enterprise Backend Architecture
Scalable backend architecture using TypeScript, NestJS, PostgreSQL and clean architecture principles.

### ⚡ AI-Enhanced Engineering Workflows
Experiments and tools focused on improving developer productivity using AI-assisted workflows.

### 🎓 Educational Platforms & LMS Solutions
Projects related to e-learning systems, virtual education and scalable educational platforms.

### 🔐 Authentication & Authorization Systems
Enterprise-ready auth systems with role-based permissions and scalable access control.

---

## 🔭 Current Focus

Currently working on:

- Scalable backend architectures
- Clean Architecture with NestJS
- Advanced PostgreSQL modeling
- AI-assisted software engineering workflows
- Enterprise-grade TypeScript applications
- Developer productivity systems

---

## 🌐 Connect With Me

<p align="left">
  <a href="https://linkedin.com/in/john-marlon-montoya-escobar-85937a163" target="_blank">
    <img src="https://skillicons.dev/icons?i=linkedin" />
  </a>

  <a href="mailto:montoyamarlon14@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" />
  </a>
</p>

---

<p align="center">
  <i>Architecture first. Scalability always.</i>
</p>
```
